...
public class CrateHologramAddLinePrompt
extends StringPrompt {
    private final Crate crate;
    private final int index;

    public CrateHologramAddLinePrompt(Crate crate, int index) {
        this.crate = crate;
        this.index = index;
    }
    ...
    public String getPromptText(@NotNull ConversationContext conversationContext) {
        return CC.YELLOW + "Type in the line you want to add to the hologram, type " + CC.RED + "cancel " + CC.YELLOW + "to cancel.";
    }
    ...
    public Prompt acceptInput(@NotNull ConversationContext context, @Nullable String s) {
        ...
        List<String> lines = this.crate.getHologramLines();
        if (lines.isEmpty()) {
            this.crate.getHologramLines().add(CC.translate(s));
        } else {
            this.replaceLine(lines, s);
        }
        player.sendMessage(CC.GREEN + "You've added the line " + CC.YELLOW + CC.translate(s) + CC.GREEN + " to the hologram.");
        this.crate.respawnHolograms();
        EditHologramLinesMenu menu = new EditHologramLinesMenu(this.crate);
        menu.selected = this.crate.getHologramsLines().size() > this.index ? this.index + 1 : this.index;
        menu.open(player);
        return END_OF_CONVERSATION;
    }
